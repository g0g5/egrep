...
name: wegrep
description: >
  Hybrid workspace code search using wegrep. Use this whenever you need to find
  code, locate definitions, search for patterns, or answer semantic questions
  about a codebase that has a wegrep index. wegrep combines BM25 keyword search
...
`wegrep` is a hybrid workspace search CLI. The default command is **query** —
...
- `.wegrep/manifest.json` — the index manifest (created by `wegrep init`)
- Provider config — either `.wegrep/provider.json` (workspace) or
  `~/.config/wegrep/provider.json` (global), created by `wegrep config`

If the index is missing, the tool will error with "index not found; run
`wegrep init` first" and exit code 3. If the provider config is missing, it
errors with exit code 4. Do not attempt to fix these — tell the user to run
`wegrep init` or `wegrep config` as appropriate.

## Usage

```
wegrep "your search query" [--top-k N] [--no-rerank] [-v | --verbose]
```
...
Use wegrep when you need to understand where something lives or how it works in
a codebase:
...
wegrep is particularly effective for questions that combine keywords with
conceptual intent, since the hybrid approach covers both exact matches and
semantic similarity.

## When NOT to Use wegrep

- To find exact string literals or grep-like pattern matches — use `rg` or
  `grep` instead. wegrep searches on chunked text and metadata, not raw file
  bytes.
...
| Exit code | Cause |
|-----------|-------|
| 3 | Index not found (`.wegrep/manifest.json` missing) |
| 4 | Provider config missing or invalid |
| 5 | Provider API error (network, auth, bad response) |
| 1 | Unexpected error |

If wegrep returns a non-zero exit code, inspect the stderr message and act
